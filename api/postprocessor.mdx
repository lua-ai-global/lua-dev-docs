...
<Warning>
**Streaming Limitation:** PostProcessors only execute on non-streaming (`generate`) requests. Channels that use streaming (WhatsApp, LuaPop, Slack, etc.) do not support post-processing. See [Channel Compatibility](/overview/postprocessors#channel-compatibility) for details.
</Warning>
