...
<Note>
**New in v3.6.0:** `lua telemetry` command to manage anonymous usage data collection.
</Note>
...
<Note>
  **New in v3.6.0:** Manage whether lua-cli sends anonymous usage data.
</Note>

Control whether lua-cli collects anonymous usage data to help improve the developer experience.
...
lua-cli collects **anonymous, non-identifiable** usage data:
...
| CLI version | Email or personal information |
| OS and Node.js version | Agent names or custom data |

All users are identified by a one-way hash of their API key — it cannot be reversed.
...
Usage:
  lua telemetry on      Enable anonymous telemetry
  lua telemetry off     Disable telemetry
...
```
  Lua CLI collects anonymous usage data to improve the developer experience.
...
| `lua telemetry` | Telemetry settings | ❌ No | Opt in/out of anonymous usage data |
