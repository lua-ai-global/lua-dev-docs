...
<Note>
  **New in v3.6.0:** 📊 **Usage telemetry** — lua-cli now collects usage data to help improve the developer experience. Opt out anytime with `lua telemetry off` or `LUA_TELEMETRY=false`.
</Note>
...
| `lua pushBackup` | ☁️ Deployment     | Push source files to S3 for restoration (`--force`, `--skipCompile`)        | ✅ Full support |
