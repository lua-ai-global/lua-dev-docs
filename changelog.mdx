...
<AccordionGroup>
  <Accordion title="Email Channel Creation Aligned With API Schema (LUA-122)">
    The `lua channels` email channel creation flow has been updated to match the current API schema:

    - **Mode selection**: Choose between "generated" (Lua creates an inbox for you) or "existing" (forward from your own email address)
    - **Display name**: The `name` field has been renamed to `displayName`
    - **Generated mode**: Creates a new email address — no forwarding setup required
    - **Existing mode**: Provides a forwarding address with setup instructions (previous behavior)

    ```bash
    # Interactive flow now asks:
    # 1. Select mode: Generated inbox or Existing email
    # 2. Enter display name
    # 3. (Existing only) Enter sender email address
    lua channels
    ```
  </Accordion>
</AccordionGroup>
...
- Added `CreateGeneratedEmailChannelResponse` interface (returns `emailAddress`)
- Added `CreateExistingEmailChannelResponse` interface (returns `forwardTo`)
- `CreateEmailChannelRequest.email` now uses `{ mode, displayName, email? }` instead of `{ name, email }`
...
<AccordionGroup>
  <Accordion title="Variable References in Primitive Config">
    You can now define primitive properties using variables and imports — not just inline literals. Previously, extracting values to a variable would cause the compiler to silently ignore them.
    ...
    export const mySkill = defineSkill({
      tools: MY_TOOLS,              // ✅ variable reference
    });
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Webhook headerSchema Now Works">
    Webhooks defining `headerSchema` previously had their header validation silently dropped due to a key mapping bug. This is now fixed — header validation schemas are correctly pushed to the server.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Batch Version Checks in Push All">
    `lua push all` now batches version lookups — one HTTP call per primitive type instead of one per entity.
...
<AccordionGroup>
  ...
  <Accordion title="MCP and Mastra Log Types">
    ...
    - 🔌 **MCP Tools**: View MCP tool execution logs with plug icon
